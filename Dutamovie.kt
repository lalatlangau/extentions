...
class DutamovieProvider : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val poster = fixUrlNull(document.selectFirst("div.poster img, img.wp-post-image")?.attr("src"))
        val tags = document.select("div.genre a, .genres a").map { it.text() }
        val year = document.selectFirst("span.year, .yearz")?.text()?.toIntOrNull()
        val description = document.selectFirst("div.entry-content p, div.description p")?.text()?.trim()
        val rating = document.selectFirst("span.rating, .rating-prc")?.text()?.toRatingInt()
        ...
        val tvType = if (document.select("div.seasons, .episode-list, #seasons").isNotEmpty()) 
            TvType.TvSeries 
        else 
            TvType.Movie
        
        return if (tvType == TvType.TvSeries) {
            val episodes = document.select("div.episode-list a, .episodes a, .eplister ul li a").mapNotNull { ep ->
    ...
    override suspend fun loadLinks(
        ...
        document.select("div.player-embed iframe, iframe[src*=player]").forEach { iframe ->
            val iframeUrl = fixUrlNull(iframe.attr("src")) ?: return@forEach
            loadExtractor(iframeUrl, subtitleCallback, callback)
        }
        
        // Extract direct download links
        document.select("div.download-links a, .download a").forEach { link ->
